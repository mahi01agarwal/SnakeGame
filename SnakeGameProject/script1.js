...
function isCollide(snake){
    ...
    if(snake[0].x>18 || snake[0].x<=0 || snake[0].y<=0 || snake[0].y>18){
        ...
        if((snake[0].x===3) && (snake[0].y>=5 && snake[0].y<=12)){
            return true;
        }
        return false;
}
...
function GameEngine(){
    ...
    while(true){
        ...
        if((food.x===3)  && (food.y>=5 && food.y<=12)){
    ...
    for(let i=0;i<snakeArr.length;i++){
        ...
        //Display Obstacles
            for(let i=5;i<=12;i=i+1){
            obstacleElement=document.createElement("img");
            obstacleElement.src="barrier.png";
            obstacleElement.style.gridRowStart=i;
            obstacleElement.style.gridColumnStart=3;
