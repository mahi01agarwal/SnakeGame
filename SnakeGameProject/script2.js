...
function isCollide(snake){
    ...
    if(snake[0].x>18 || snake[0].x<=0 || snake[0].y<=0 || snake[0].y>18){
        ...
        if((snake[0].x===3||snake[0].x===15) && (snake[0].y>=5 && snake[0].y<=12)){
            return true;
        }
        return false;
}
function GameEngine(){
    ...
    scorebox.innerHTML = "Score: "+score;
        if(score>hiscore2){
            hiscore2 = score;
            localStorage.setItem("hiscore2", JSON.stringify(hiscore2));
            hiscoreBox.innerHTML = "Hi-Score: " + hiscore2;
        }
        scorebox.innerHTML = "Score: " + score;
    snakeArr.unshift({x: snakeArr[0].x + inputDir.x, y: snakeArr[0].y + inputDir.y});
    while(true){
        food = {x:Math.round(6+10*Math.random()),y:Math.round(6+10*Math.random())};
        if((food.x===3||food.x===15)  && (food.y>=5 && food.y<=12)){
...
if(hiscore2 === null){
    hiscore2val = 0;
    localStorage.setItem("hiscore2", JSON.stringify(hiscore2val))
}
else{
    hiscoreBox.innerHTML = "Hi-Score: " + hiscore2;
}
